...
namespace Excel2Csv
{
    public static class EpplusCsvConverter
    {
        ...
        public static byte[] ConvertToCsv(this ExcelWorksheet worksheet, string filePath = "")
        {
            var maxColumnNumber = worksheet.Dimension.End.Column;
            var currentRow = new List<string>(maxColumnNumber);
            var totalRowCount = worksheet.Dimension.End.Row;
            ...
            using (var writer = new StreamWriter(memory, Encoding.ASCII))
            {
                ...
                {
                    BuildRow(worksheet, currentRow, currentRowNum, maxColumnNumber);
                    WriteRecordToFile(currentRow, writer, currentRowNum, totalRowCount, worksheet.Name, filePath);
                    currentRow.Clear();
                    currentRowNum++;
                }
            }
        ...
        public static bool IsWorksheetEmpty(this ExcelWorksheet worksheet)
        {
        ...
        private static void WriteRecordToFile(List<string> record, StreamWriter sw, int rowNumber, int totalRowCount, string worksheetName = "", string filePath = "")
        {
            if (!string.IsNullOrEmpty(worksheetName) && !string.IsNullOrEmpty(filePath))
            {
                record.Insert(0, worksheetName);
            }
